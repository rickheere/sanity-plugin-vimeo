...
export function SkipButtons({ index, maxIndex, nextLink, previousLink }) {
  let firstIndex = -1
  let secondIndex = 0
  let thirdIndex = -1
...
function Box({ active, deactivated = false, link, children }) {
  ...
  return (
    <div {...{ className }} onClick={link}>
      {children}
    </div>
  )

  function addClass(classToAdd) {
    return `${className} ${classToAdd}`
  }
}

function getArrowColor(disabled) {
  const dark = '#383D42'
  const light = '#EFF0F2'
